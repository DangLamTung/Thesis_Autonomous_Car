...
def print_time( threadName,image,Lat,Lon,pitch):
    global result
    ...
    x,y = global2pix(Lat, Lon+100,Lat_Or,Lon_Or)
    # print(x,y)
    ...
    occupancy_map = np.zeros((6,20))
    dis_map = np.ones((6,20))*1000
    # p1,p2,p3,p4 = get_map_pos(pitch, x, y)
    # p5, r_p5 = get_map_loc(pitch, x, y, 3, 14)
    
    lon_goal, lat_goal = pix2global(100,100,map_png.shape[1],map_png.shape[0])
    ...
    for i in range(6):
        for j in range(20):
            ...
            cv2.line(result, (0, (i+1)*10), (200, (i+1)*10), (255, 255,255), thickness=line_thickness)
            cv2.line(result, (j*10, 0), (j*10, 66), (255, 255,255), thickness=line_thickness)
    ...
    for i in range(6):
        for j in range(20):
            if(road[i][j] == 0):
                ...
                dis_x, dis_y = ConvertGPStoUCS(lat_goal,lon_goal,lat_now, lon_now - 100) 
                l2_dis = np.sqrt(dis_x**2 + dis_y**2)
                dis_map[i][j] = l2_dis
    print(road)
    g.walls = wall
    ...
    if(found_way == 1):
        ...
        for i in range(len(path)):
            if(i>1):
                ...
                cv2.line(result, point1, point2, (0, 255, 0), thickness=line_thickness)
