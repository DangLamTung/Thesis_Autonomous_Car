...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #Show map #######################################################################
        #Map timer ######################################################################
        self.timer_map = QtCore.QTimer(self)
        self.timer_map.setInterval(10) # in milliseconds
        self.timer_map.start()
        self.timer_map.timeout.connect(self.onNewData)
    ...
    def onNewData(self):
        ...
        # print(data_str, end='') #print the incoming       # read line (single value) from the serial port
            try:
                ...
                self.widget_3.setAngle(float(X[2]))
                self.state.update_real(float(X[0]),float(X[1]),float(X[2]),0)
                delta, current_target_idx = stanley_control(self.state, self.cx, self.cy, self.cyaw, self.target_idx)
                print(delta, current_target_idx)
                ...
                if(float(X[0])< 5):
                    send_control(int(delta+90),100)
                else:
                    send_control(int(delta+90),0)
                # data.append(x_[1])
