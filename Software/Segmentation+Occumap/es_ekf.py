...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + 0.5*(delta_t**2)*(np.dot(Rot_mat,imu_f.data[k - 1])+ g)
    v_est[k] = v_est[k - 1] + delta_t * (Rot_mat.dot(imu_f.data[k - 1]) + g)
    ...
    F[3:6, 6:9] = Rot_mat.dot(-skew_symmetric(imu)) * delta_t

    # 2. Propagate uncertainty
    Q = np.eye(6)
    Q[0:3,0:3] = var_imu_f * Q[0:3, 0:3]
    Q[0:3,0:3] = var_imu_w * Q[0:3, 0:3]
    Q = (delta_t ** 2) * Q
    p_cov[k] = np.dot(np.dot(F,p_cov[k-1]),np.transpose(F)) + np.dot(np.dot(l_jac,Q),np.transpose(l_jac))
