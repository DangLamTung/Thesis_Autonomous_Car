...
def draw_map( threadName,pitch, Lat, Lon,global_path):
    global map_png, result
    
    demo = np.copy(map_png)
    x, y = int((Lon - Lon_Or)*map_png.shape[1]/(106.66226 - 106.65644)),int((Lat_Or - Lat)*map_png.shape[0]/(10.77654 - 10.77023))
    ...
    while True:
        if not connected:
            ...
            try:
                ...
                # rl = ReadLine(ser) 
                ser1 = serial.Serial('COM10', 115200, timeout=5.0)
                sio = io.TextIOWrapper(io.BufferedRWPair(ser1, ser1))
                connected = True
            ...
            try:
                ...
                X =  (reading.decode().replace('\x00','').replace('\x00','')).split(' ')
                # float(X[0])
                # float(X[1])
                _thread.start_new_thread( draw_map, ("Thread-1", float(X[0]), msg.latitude, msg.longitude,global_path) )
                time.sleep(0.2)
    ...
    while True:
        ...
        if evt:
            if evt.get(socket) == zmq.POLLIN:
                ...
                _thread.start_new_thread( draw_map, ("Thread-1", image,float(arr[4])*180/np.pi, float(arr[1]), float(arr[2]),global_path) )
                #_thread.start_new_thread( draw_map, ("Thread-2", image,float(arr[1]), float(arr[2]),float(arr[4])*180/np.pi,global_path) )
